...
func (gm *GameManager) DrawRect(mapChunk *MapChunk, x1, y1, x2, y2 int) {
	...
	for x := 0; x < len(mapChunk.TileMap); x++ {
		for y := 0; y < len(mapChunk.TileMap[0]); y++ {
			// If x or y is the rectangle border, draw a wall.
			if (x == x1 || x == x2) && (y >= y1 && y <= y2) { // Left or right edges
				mapChunk.TileMap[x][y][0] = Tile{TileType: "wall"}
